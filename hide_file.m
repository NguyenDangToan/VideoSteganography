function varargout = hide_file(varargin)
% HIDE_FILE MATLAB code for hide_file.fig
%      HIDE_FILE, by itself, creates a new HIDE_FILE or raises the existing
%      singleton*.
%
%      H = HIDE_FILE returns the handle to a new HIDE_FILE or the handle to
%      the existing singleton*.
%
%      HIDE_FILE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HIDE_FILE.M with the given input arguments.
%
%      HIDE_FILE('Property','Value',...) creates a new HIDE_FILE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before hide_file_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to hide_file_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help hide_file

% Last Modified by GUIDE v2.5 30-Sep-2019 06:53:54

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @hide_file_OpeningFcn, ...
                   'gui_OutputFcn',  @hide_file_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before hide_file is made visible.
function hide_file_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to hide_file (see VARARGIN)

% Choose default command line output for hide_file
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes hide_file wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = hide_file_OutputFcn(hObject, eventdata, handles) 
